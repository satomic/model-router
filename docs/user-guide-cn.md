...
左侧导航「管理」分组的第一项「策略视图」打开 `/topology`，默认显示策略索引与关联来源、目标数量，
**不加载用户、用户绑定或用户 Key**，也不把所有策略的连线混在一张图里。可在「策略」「主体」「模型」
索引中搜索，每页最多 24 项；主体索引包括已发现但未绑定的范围。

点击一项后进入局部关系图：从关联来源、当前节点向右展开关联目标，按画布实际宽度自动换行，
不再限制每行三个，也不再对关系节点分页。窄屏会将阶段及节点折行，保持卡片文字可读，更多内容直接滚动查看。
连线沿卡片间隙绘制。点击相邻节点继续追踪，使用返回箭头或路径导航回退。成员列表和总览索引仍保留各自分页。
悬停时强调对应关系，点击连线或当前节点查看详情；详情位于画布下方，不会挤窄画布。支持缩放、平移和
适应当前图，修改仍在原配置页面完成。

点击企业可查看其组织和 Enterprise Team，后者直接隶属企业，不伪造组织到团队的关系。
点击组织或企业团队后才加载成员，每页最多 50 人。点击成员后才计算其权限和读取 Key，直接进入该用户的
「有效模型权限」图；路径导航可返回这个用户的其他策略。一次仅保留一个用户，切换时替换而非累积路径。
「策略总览」、切换成员页或关闭成员列表会清除用户路径。
「已知用户」提供注册过或配置中引用的账号列表，按需分页读取，不是完整 GitHub 成员清单。

节点以颜色和图标区分：企业蓝、组织青绿、团队玫红、用户灰蓝、策略琥珀、模型绿色；当前节点带有突出边框。
虚线表示未生效。创建 Key 的规则取 OR，Scope 授权在已配置层级之间取 AND，
模型组取并集。**Key 到模型的连线仅表示 Scope 候选，最终可用模型仍为所有者模型策略与 Key Scope 的交集。**
管理员豁免、空模型组和无绑定时放行的规则均可在详情中查看。

所选用户的 Key 创建、Scope 编辑权限及「有效模型权限」来自后端实际计算；无法确认时显示「未知」，而不是「拒绝」。GitHub 数据不完整、
过期或请求失败会提示，已保存的配置仍可查看。「重新加载」读取已保存的最新状态，不强制重建 GitHub 缓存。
展开「数据状态」可逐条查看具体企业、缓存范围或失败接口、数据类别、已加载数量、原始错误及获取时间。
例如组织截断会显示「已加载 200 / 997 个组织」，Teams 接口的 404 会作为另一条问题列出；
成员缓存截断仅报告缓存人数，不把未知总人数当作已知。Token 不匹配与缓存过期也分别说明。
详细语义见 [全局策略拓扑](access-control.md#global-policy-topology)。
...
**BYOK 拦截**：开启后，**隶属于**共享池仍有余额的企业的用户发来的请求**既不路由，也不经过决策模型**，而是
以普通 assistant 消息返回一段提示（默认英文，可自定义，支持 `{enterprise}`、`{remaining}` 等占位
符），流式与非流式、OpenAI 与 Anthropic 协议都一样。用户属于哪个企业以席位列表为准；GitHub 不提供席位
列表时，用密钥策略的成员缓存判断；无法归属到任何企业的用户直接放行，不会被别人的共享池拦下。共享池用尽、
快照过期或轮询停止后自动放行——任何不确定的情况都放行，拦截是为了省钱，不能成为开发者干不了活的原因。
「剩余额度低于此值即解除拦截」可以让拦截在 GitHub 真正开始计费之前稍早一点松开，抵消账单数据的滞后。

勾上**按用户判断**后，轮询还会读取 GitHub 的用户级预算，并且**用户自身预算优先于共享池**：预算还有剩余就
勝回 Copilot（返回另一段带 `{budget_remaining_usd}` 等占位符的提示），即使共享池已经用尽；预算用尽则
GitHub 会拦下该用户，BYOK 放行。没有用户级预算的用户回到共享池判断。每个企业的卡片下会列出所有席位持有
者的预算、消耗、剩余空间和判定结果（共享池 / 自身预算 / 放行），可按登录名筛选。

被拦截的请求仍会写入调用链，模型与决策两列都显示 `ai-credits-gate`，详情里写明是哪个企业、因共享池还是自身
预算而拦截、剩余多少。详细推导与 GitHub API 的能力边界见 [Copilot AI credits](ai-credits.md)。
