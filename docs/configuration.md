...
(`providers` / `models` / `strategy` / `session` / `ai_router` / `rules` / `model_groups` /
`model_policy`) and **Access control** (`auth`). Each has a sticky save bar below its sub-page navigation — **when there are unsaved
...
The 5 sub-pages of "Routing configuration":
...
| Routing strategy | `strategy` / `session` / `ai_router` | `rule`, `ai`, or `rule-then-ai` (both active: the rules decide when one matches, and only an unmatched request pays for a decision call); the stickiness toggle and capacity (one decision per interaction / per session); the decision model, its provider, timeout and prompt truncation length, or — ticking "Use a System One decision model" — `ai_router.decision_engine: typesafe` (TypeSafe Jev, with its key/model) or `laya` (a self-hosted Laya, with its address) (see [router logic](router-logic.md)); the **AI decision prompt** (`ai_router.decision_prompt`) editor plus a preview rendered against the real model catalog |
| Rule routing | `rules` | keywords / prompt length, matched in order |
| Model policy | `model_groups` / `model_policy` | named model groups ticked off the catalog, and which group each user / team / organization gets — see [model policy](model-policy.md) |
